...
abbr_month <- function(date) {
  if (!inherits(date, "Date")) {
    stop("Not a date object", call. = FALSE)
  }

  levels <- format(
    seq(as.Date("2018-01-01"), as.Date("2018-12-01"), "1 month"),
    "%b"
  )

  factor(format(date, "%b"), ordered = TRUE, levels = levels)
}
...
check_response <- function(link) {
  ...
  if (!response$status_code == 200) {
    warning(
      paste0(
        "Non successful http request. Target server returning a ",
  ...
  if (grepl("shutdown", response$url)) {
    stop(
      "Data source is currently unavailable due to a US government shutdown",
      call. = FALSE
    )
  }
...
with_cache <- function(
  ...
  write_function = function(data, file) {
  ...
  if (!curl::has_internet()) {
    message(
      "A working internet connection is required to download and import the climate indices."
    )
    return(NULL)
  }
  data <- unmemoised(...)
